  ...
  filter(row: any, fieldOrLabel: string, operator: string, value: string) {
    ...
    switch (operator) {
      ...
      case "!=":
        return row[field] != value;
      case "contains":
        return row[field].toLowerCase().indexOf(value.toLowerCase()) >= 0;
      case "!contains":
        return row[field].toLowerCase().indexOf(value.toLowerCase()) < 0;
      case "startsWith":
        return _.startsWith(row[field].toLowerCase(), value.toLowerCase());
      case "after":
        return true;
    }
...
export default class Demo3 extends React.Component<any, any> {
    ...
    if (!_.isString(data.value)) {
      ...
      return (
        <div className="day-picker-selection">
          {/* @ts-ignore */}
          <DayPicker
            onDayClick={(day: any) => pick(day.toLocaleDateString())}
          />
        </div>
      );
    }
  ...
  onParseOk(expressions: Expression[]) {
    var newData = new CustomResultProcessing(this.options).process(
      data,
      expressions
    );
      ...
      <div className="main-container">
        <h3>
          ...
          <a
            ...
            href="https://github.com/nhabuiduc/react-filter-box/blob/master/js-example/src/demo3.js"
          >
            Source
          </a>
        </h3>
        ...
        <ReactFilterBox
          ...
          customRenderCompletionItem={this.customRenderCompletionItem.bind(
            this
          )}
